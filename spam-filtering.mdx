...
Zerion's API includes a built-in spam detection system that flags low-quality or deceptive assets. Each asset includes an `is_trash` boolean in the response, which you can use to filter out spam tokens across endpoints like `/positions` and `/transactions` without building your own detection logic.
...
- **Transactions**: defaults to `no_filter` (spam included) — transactions reflect historical on-chain activity, so spam received in the past is shown by default
</Note>
